...
def is_running():
    ...
    return 'hostapd' in processes or 'dnsmasq' in processes
...
class AccessPoint:
    ...
    def _start_router(self):
        ...
        if self.inet is not None:
            ...
            if len(r.strip()) > 0:
                logging.debug(r.strip())
            _execute_shell('iptables -t nat -A POSTROUTING -o {} -j MASQUERADE'.format(self.inet))
            _execute_shell(
                'iptables -A FORWARD -i {} -o {} -j ACCEPT -m state --state RELATED,ESTABLISHED'
                    .format(self.inet, self.wlan))
            _execute_shell('iptables -A FORWARD -i {} -o {} -j ACCEPT'.format(self.wlan, self.inet))

        # allow traffic to/from wlan
        _execute_shell('iptables -A OUTPUT --out-interface {} -j ACCEPT'.format(self.wlan))
        _execute_shell('iptables -A INPUT --in-interface {} -j ACCEPT'.format(self.wlan))
    ...
    def _stop_router(self):
        ...
        _execute_shell('ifconfig mon.' + self.wlan + ' down')
        ...
        if self.wlan is not None:
            _execute_shell('iptables -D OUTPUT --out-interface {} -j ACCEPT'.format(self.wlan))
            _execute_shell('iptables -D INPUT --in-interface {} -j ACCEPT'.format(self.wlan))
        _execute_shell('iptables --table nat --delete-chain')
        _execute_shell('iptables --table nat -F')
